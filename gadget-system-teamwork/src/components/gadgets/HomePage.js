    ...
    let gadgets = this.state.lastAddedGadgets.map(gadget => {
      return (
        <div className='homeGadgets' key={gadget._id}>
          ...
          <Link className="homeLink" to={`/gadgets/delete/${gadget._id}`}>Delete</Link>
          <Link className="homeLink" to={`/gadgets/edit/${gadget._id}`}>Edit</Link>
          <Link className="homeLink" to={`/gadgets/details/${gadget._id}`}>Details</Link>
        </div>
      )
